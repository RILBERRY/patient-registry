...
import GetPatient from '../../inc/GetPatients.js'
export default {
    props: ['id'],
    data () {
        return {
    ...
    setup(props){
        const { Patient, err, load} = GetPatient(props.id)
        load()
        console.log(Patient)
        return {Patient, err}
    },
    methods: {
        handelSubmit(){
            ...
            || this.AtollInpErr || this.IslandInpErr || this.CountryInpErr){
                ...
                axios.put("http://127.0.0.1:8000/api/patient/",this.newQuery).then((res)=>{
        ...
        DataBind(){
            this.patientName = this.patientName == '' ? this.Patient.patientName : this.patientName
            this.DOB = this.DOB == '' ? this.Patient.DOB : this.DOB
            this.nationalID = this.nationalID == '' ? this.Patient.nationalID : this.nationalID
            this.floor = this.floor == '' ? this.Patient.floor : this.floor
            this.houseName = this.houseName == '' ? this.Patient.houseName : this.houseName
            this.postCode = this.postCode == '' ? this.Patient.postCode : this.postCode
            this.street = this.street == '' ? this.Patient.street : this.street
            this.Atoll = this.atoll == '' ? this.Patient.atoll : this.atoll
            this.Island = this.islandName == '' ? this.Patient.islandName : this.islandName
            this.Country = this.country == '' ? this.Patient.country : this.country
        },
        activate() {
